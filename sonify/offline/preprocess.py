...
def get_raw():
    cwd = os.getcwd()
    filename = str(samp_freq) + '_Hz_' + str(os.path.splitext(dataset_file)[0])+'_raw.fif'
    file_path = os.path.join(cwd, 'downsampled_eeg', dataset_folder, filename)
    if(os.path.isfile(file_path)):
        print('Downsampled eeg file already existing. Loading...')
        return load_downsampled_raw()
    else:
        print('The files will first be downsampled to ', samp_freq, 'Hz')
        return downsample_to_file(load_raw())

# Load raw data from brainvision files
def load_raw():
    cwd = os.getcwd()
    file_path = os.path.join(cwd, dataset_folder, dataset_file)
...
def downsample(raw):
    ...
    raw_downsampled = raw.copy().resample(samp_freq)
...
def downsample_to_file(raw):
    ...
    # Save to file
    cwd = os.getcwd()
    dir = os.path.join(cwd, 'downsampled_eeg', dataset_folder)
